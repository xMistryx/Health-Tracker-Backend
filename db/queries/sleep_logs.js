...
export async function addSleepEntry(userId, { date, sleep_type, start_time, end_time, duration }) {
    const sql = `
        ...
        RETURNING *
    `;
    const { rows: [sleep] } = await db.query(sql, [userId, date, sleep_type, start_time, end_time, duration]);
    return sleep;
}

export async function updateSleepEntry(id, { sleep_type, start_time, end_time, duration }) {
    const sql = `
        UPDATE sleep_logs
        ...
        RETURNING *
    `;
    const { rows: [sleep] } = await db.query(sql, [id, sleep_type, start_time, end_time, duration]);
    return sleep;
}
